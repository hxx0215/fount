...
import { available } from '../../../../vendor/exec.mjs'
...
角色: 1<inline-js>'0'.repeat(308)</inline-js>
${args.UserCharname}: 反向输出\`never gonna give you up\`。
角色: 好哒，<inline-js>'never gonna give you up'.split('').reverse().join('')</inline-js>！
${args.UserCharname}: 97的32次方是多少？
角色: 是<inline-js>97n**32n</inline-js>哦？
${args.UserCharname}: js中\`![]+[]\`是什么？
角色: 是<inline-js>![]+[]</inline-js>！
${available.powershell || available.pwsh ? `\
${args.UserCharname}: 我系统盘是哪个？
角色: 是<inline-pwsh>$env:SystemDrive</inline-pwsh>哦。
` : available.bash ? `\
${args.UserCharname}: 我家目录在哪？
角色: 在<inline-bash>echo $HOME</inline-bash>哦。
` : available.sh ? `\
${args.UserCharname}: 我家目录在哪？
角色: 在<inline-sh>echo $HOME</inline-sh>哦。
` : ''}\
...
角色: <run-js>await add_files('E:\\paper.pdf','~/Desktop/data.zip')</run-js>
${args.UserCharname}: 帮我下载http://host/file.txt然后发来。
角色: <run-js>await add_files('http://host/file.txt')</run-js>
]
